    ...
    <div class="col-md-6">
        <div class="card p-4 border-0 shadow-sm" style="background-color: #1a233a; border-radius: 12px;">
            <div class="d-flex align-items-center gap-3 mb-4">
                ...
                <h5 class="fw-bold text-white mb-0">Tambah Kelas Baru</h5>
            </div>
            ...
            <form action="{{ route('admin.kelas.store') }}" method="POST">
                ...
                <div class="mb-3">
                    ...
                    <select name="tingkat" class="form-select custom-input @error('tingkat') is-invalid @enderror">
                        ...
                        <option value="X" {{ old('tingkat') == 'X' ? 'selected' : '' }} style="background-color: #131926;">X (Sepuluh)</option>
                        <option value="XI" {{ old('tingkat') == 'XI' ? 'selected' : '' }} style="background-color: #131926;">XI (Sebelas)</option>
                        <option value="XII" {{ old('tingkat') == 'XII' ? 'selected' : '' }} style="background-color: #131926;">XII (Dua Belas)</option>
                    </select>
                    @error('tingkat')
                ...
                <div class="mb-3">
                    ...
                    <input type="text" name="jurusan" class="form-control custom-input @error('jurusan') is-invalid @enderror"
                        placeholder="Contoh: TKJ, RPL, Akuntansi" value="{{ old('jurusan') }}">
                    @error('jurusan')
                        <div class="invalid-feedback small">{{ $message }}</div>
                    @enderror
                </div>
    ...
    @enderror
                ...
                <div class="mb-4">
                    ...
                    <input type="text" name="wali_kelas" class="form-control custom-input @error('wali_kelas') is-invalid @enderror"
                        placeholder="Nama lengkap guru wali kelas" value="{{ old('wali_kelas') }}">
                    @error('wali_kelas')
                        <div class="invalid-feedback small">{{ $message }}</div>
                    @enderror
                </div>
